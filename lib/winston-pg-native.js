...
class Postgres extends Transport {
  ...
  log(args, callback) {
    ...
    return pool.connect().then(client => client
      .query(sqlStatement || table.insert(table.timestamp.value('NOW()'), table.level.value(level), table.message.value(message), table.meta.value(meta)).toQuery())
      .then(() => {
        client.release();
  ...
  query(args, callback) {
    ...
    const fields = options && options.fields instanceof Array ? options.fields.map(field => table[field]) : [table.star()];
